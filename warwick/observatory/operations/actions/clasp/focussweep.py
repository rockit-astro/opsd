...
CONFIG_SCHEMA = {
    ...
    'required': ['ra', 'dec', 'min', 'max', 'step', 'camera'],
    'properties': {
        'type': {'type': 'string'},
        'ra': {
            'type': 'number',
...
class FocusSweep(TelescopeAction):
    ...
    def run_thread(self):
        ...
        expected_next_exposure = datetime.datetime.utcnow() + \
            datetime.timedelta(seconds=self.config[self._camera_id]['exposure'] + MAX_PROCESSING_TIME)
    ...
    def received_frame(self, headers):
        ...
        with self._wait_condition:
            if 'MEDHFD' in headers and 'HFDCNT' in headers and self._header_keyword in headers:
                print('got hfd', headers['MEDHFD'], 'from', headers['HFDCNT'], 'sources')
                self._focus_measurements[headers[self._header_keyword]] = (headers['MEDHFD'], headers['HFDCNT'])
            else:
                print('Headers are missing MEDHFD, HFDCNT, or ' + self._header_keyword)
                print(headers)
