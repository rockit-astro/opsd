...
def __set_focus(channel, position):
    try:
        with daemons.rasa_focus.connect(timeout=FOCUS_TIMEOUT) as focusd:
            ...
            if status != FocCommandStatus.Succeeded:
                ...
                log.error('opsd', 'Failed to set focuser position')
                return False
    except Pyro4.errors.CommunicationError:
        print('Failed to communicate with focuser daemon')
        log.error('opsd', 'Failed to communicate with focuser daemon')
        return False
    except Exception as e:
        print('Unknown error while configuring focuser')
        print(e)
        log.error('opsd', 'Unknown error while configuring focuser')
        return False
    return True

def __start_exposure(config):
    try:
...
class FocusSweep(TelescopeAction):
    ...
    def run_thread(self):
        ...
        if not __set_focus(self.config['channel'], current_focus):
            self.status = TelescopeActionStatus.Error
            return

        # Configure the camera then take the first exposure to start the process
        if not __start_exposure(self.config['rasa']):
            self.status = TelescopeActionStatus.Error
            return
