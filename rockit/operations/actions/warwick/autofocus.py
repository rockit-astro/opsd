...
class AutoFocus(TelescopeAction):
    ...
    def task_labels(self):
        ...
        if self._progress <= Progress.MeasureInitialHFD:
            subtasks.append('Measure initial HFD')
        if self._progress <= Progress.FindPosition:
            subtasks.append('Find position on V curve')
        if self._progress <= Progress.FindJumpHFD:
            subtasks.append(f'Searching for HFD {CONFIG["target_hfd"]}')
        if self._progress <= Progress.FindPosition:
            subtasks.append('Measuring HFD for final move')
        if self._progress <= Progress.MeasureFinalHFD:
            subtasks.append('Measuring final HFD')
