...
from .camera_helpers import cam_is_idle, cam_status, cam_stop, cam_take_images
from .mount_helpers import mount_offset_radec, mount_slew_radec
...
class FieldAcquisitionHelper:
    ...
    def acquire_field(self, ra_degrees, dec_degrees, exposure_seconds, threshold_arcsec=5):
        ...
        while not self._parent_action.aborted and self._parent_action.dome_is_open:
            ...
            # Offset telescope
            if not mount_offset_radec(self._parent_action.log_name,
...
class CameraWrapper:
    ...
    def update(self):
        ...
        if self.status == CameraWrapperStatus.Stopping:
            ...
            if cam_is_idle(self._log_name, self.camera_id, status):
                self.status = CameraWrapperStatus.Stopped
                return
        ...
        # Try stopping the camera and see if we can recover on the next update loop
        log.warning(self._log_name, f'Camera {self.camera_id} has timed out, stopping camera')
        cam_stop(self._log_name, self.camera_id)
