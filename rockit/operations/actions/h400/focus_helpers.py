...
from rockit.focuser.aaf5 import FocuserStatus, CommandStatus as FocCommandStatus
...
def focus_get(log_name):
    ...
    try:
        with daemons.halfmetre_focuser.connect() as focusd:
            ...
            if status['status'] in [FocuserStatus.Disabled, FocuserStatus.Initializing]:
...
def focus_set(log_name, position, timeout=FOCUS_TIMEOUT):
    ...
    try:
        with daemons.halfmetre_focuser.connect(timeout=timeout) as focusd:
            ...
            status = focusd.move(position, relative=False)
