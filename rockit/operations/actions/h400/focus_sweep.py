...
class FocusSweep(TelescopeAction):
    ...
    Example block:
    {
        ...
        "camera": {
            "exposure": 1,
            "filter": "L" # Optional: defaults to L
        ...
        "pipeline": {
           "prefix": "focussweep",
           "archive": ["H400"] # Optional: defaults to "camera"
    ...
    def received_frame(self, headers):
        ...
        with self._wait_condition:
            if 'MEDHFD' in headers and 'HFDCNT' in headers and 'TARGFOC' in headers:
                print('got hfd', headers['MEDHFD'], 'from', headers['HFDCNT'], 'sources')
                self._focus_measurements[headers['TARGFOC']] = (headers['MEDHFD'], headers['HFDCNT'])
            else:
                print('Headers are missing MEDHFD, HFDCNT, or TARGFOC')
                print(headers)
