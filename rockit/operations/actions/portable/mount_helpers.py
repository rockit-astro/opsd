...
def mount_init(log_name):
    ...
    try:
        with daemons.portable_telescope.connect() as lmountd:
...
def mount_home(log_name):
    """Homes the mount"""
    try:
        with daemons.portable_telescope.connect(timeout=HOME_TIMEOUT) as lmountd:
...
def mount_park(log_name):
    ...
    try:
        with daemons.portable_telescope.connect(timeout=PARK_TIMEOUT) as lmountd:
