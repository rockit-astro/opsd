...
class FocusSweep(TelescopeAction):
    ...
    Example block:
    {
        ...
        "pipeline": {
           "prefix": "focussweep",
           "archive": ["814"] # Optional: defaults to "camera"
    ...
    def task_labels(self):
        ...
        if self._progress < Progress.Focusing:
            ...
            tasks.append(f'Run Focus Sweep ({len(self._focus_measurements) + 1} / {count})')
    ...
    def run_thread(self):
        ...
        if not cam_take_images(self.log_name, 1, self.config['camera']):
            mount_stop(self.log_name)
            self.status = TelescopeActionStatus.Error
            return

        expected_next_exposure = Time.now() + (self.config['camera']['exposure'] + MAX_PROCESSING_TIME) * u.s
        ...
        while True:
            ...
            step = int(round((current_focus - self.config['min']) / self.config['step']))
            if step in self._focus_measurements:
    ...
    def received_frame(self, headers):
        ...
        with self._wait_condition:
            if 'MEDHFD' in headers and 'HFDCNT' in headers and 'TELFOC' in headers:
                step = int(round((headers['TELFOC'] - self.config['min']) / self.config['step']))
                print('got hfd', headers['MEDHFD'], 'from', headers['HFDCNT'], 'sources for step', step)
                self._focus_measurements[step] = (headers['MEDHFD'], headers['HFDCNT'])
            else:
