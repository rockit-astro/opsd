...
def mount_status(log_name):
    ...
    try:
        with daemons.ngts_m06_telescope.connect() as teld:
...
def mount_init(log_name):
    ...
    try:
        with daemons.ngts_m06_telescope.connect(timeout=INIT_TIMEOUT) as teld:
...
def tel_home(log_name):
    """Homes the telescope"""
    try:
        with daemons.ngts_m06_telescope.connect(timeout=HOME_TIMEOUT) as teld:
...
def mount_slew_radec(log_name, ra, dec, tracking, timeout=SLEW_TIMEOUT):
    ...
    try:
        with daemons.ngts_m06_telescope.connect(timeout=timeout) as teld:
            if tracking:
...
def mount_park(log_name):
    """Slew the telescope to the default park position"""
    try:
        with daemons.ngts_m06_telescope.connect(timeout=PARK_TIMEOUT) as teld:
            status = teld.park(PARK_NAME)
            if status != TelCommandStatus.Succeeded:
                log.error(log_name, 'Failed to park telescope')
                return False
            return True
    except Pyro4.errors.CommunicationError:
        log.error(log_name, 'Failed to communicate with telescope daemon')
        ...
        log.error(log_name, 'Unknown error while parking telescope')
        traceback.print_exc(file=sys.stdout)
        return False
